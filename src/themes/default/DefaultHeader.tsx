      ...
      <div className="max-w-7xl mx-auto px-4 py-3">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href={h.session?.user ? "/hesabim" : "/giris"} className="hidden sm:flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
            ...
            {h.session?.user && (
              <Link href="/hesabim/favorilerim" className="relative hidden sm:flex items-center text-muted-foreground hover:text-foreground">
                ...
                {h.mounted && h.wishlistCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-danger text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center">{h.wishlistCount}</span>
                )}
              </Link>
            )}
            <button onClick={h.openCart} className="relative flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
              ...
              {h.mounted && h.itemCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 bg-primary text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center">{h.itemCount}</span>
              )}
            </button>
            <button onClick={h.toggleMobileMenu} className="md:hidden p-2 text-muted-foreground">
      ...
      {h.mobileMenuOpen && (
        <div className="md:hidden border-t border-border bg-background">
          <div className="p-4 border-b border-border">
            <input type="text" placeholder={t("mobileSearchPlaceholder")} className="w-full pl-10 pr-4 py-2.5 border border-border rounded-lg text-sm" value={h.query} onChange={(e) => h.handleSearch(e.target.value)} />
          </div>
          <nav className="p-4 space-y-3">
            {[
              ...
              <Link key={item.href} href={item.href} className="block text-sm text-foreground hover:text-primary py-1" onClick={h.closeMobileMenu}>{item.label}</Link>
            ))}
          </nav>
        </div>
      )}
