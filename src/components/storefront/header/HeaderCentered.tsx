...
export default function HeaderCentered() {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === "Enter" && query.length >= 2) { e.preventDefault(); addRecentSearch(query); closeSearch(); router.push(`/arama?q=${encodeURIComponent(query)}`); }
    ...
    <header className="sticky top-0 z-50 bg-background border-b border-border">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex flex-col items-center gap-3">
          <Link href="/" className="text-3xl font-bold text-primary">Pixfora</Link>

          {/* Actions row */}
          <div className="flex items-center justify-between w-full">
            {/* Left: Account */}
            <div className="flex items-center gap-3">
              <Link href={session?.user ? "/hesabim" : "/giris"} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
              ...
              {session?.user && (
                <Link href="/hesabim/favorilerim" className="relative flex items-center text-muted-foreground hover:text-foreground">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                  </svg>
                  {mounted && wishlistCount > 0 && (
                    <span className="absolute -top-1.5 -right-1.5 bg-danger text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center">{wishlistCount}</span>
                  )}
                </Link>
              )}
            </div>
            ...
            <div ref={searchRef} className="hidden md:block flex-1 max-w-xl mx-8 relative">
              <div className="relative">
                <input type="text" placeholder={t("searchPlaceholder")}
                  ...
                  value={query} onChange={(e) => handleSearch(e.target.value)} onFocus={handleFocus} onKeyDown={handleKeyDown} />
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
              {isOpen && <SearchDropdown query={query} suggestions={suggestions} isLoading={isLoading} recentSearches={recentSearches} onClose={closeSearch} />}
            </div>

            {/* Right: Cart */}
            <div className="flex items-center gap-3">
              <button onClick={openCart} className="relative flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" />
                </svg>
                <span className="hidden lg:inline">{tc("cart")}</span>
                {mounted && itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-primary text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center">{itemCount}</span>
                )}
              </button>
      ...
      <nav className="hidden md:block border-t border-border">
        <div className="max-w-7xl mx-auto px-4">
          <ul className="flex items-center justify-center gap-8 py-2.5 text-sm">
            <li><Link href="/kategori" className="font-medium text-foreground hover:text-primary transition-colors">{t("allCategories")}</Link></li>
            <li><Link href="/markalar" className="text-muted-foreground hover:text-primary transition-colors">{tc("brands")}</Link></li>
            <li><Link href="/firsatlar" className="text-danger font-medium hover:text-danger/80 transition-colors">{t("deals")}</Link></li>
            <li><Link href="/yeni-urunler" className="text-muted-foreground hover:text-primary transition-colors">{t("newProducts")}</Link></li>
            <li><Link href="/cok-satanlar" className="text-muted-foreground hover:text-primary transition-colors">{t("bestSellers")}</Link></li>
            <li><Link href="/blog" className="text-muted-foreground hover:text-primary transition-colors">{t("blog")}</Link></li>
            <li><Link href="/iletisim" className="text-muted-foreground hover:text-primary transition-colors">{t("contact")}</Link></li>
          </ul>
        </div>
      </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-border bg-background">
          <div className="p-4 border-b border-border">
            <input type="text" placeholder={t("mobileSearchPlaceholder")} className="w-full pl-10 pr-4 py-2.5 border border-border rounded-lg text-sm bg-background" value={query} onChange={(e) => handleSearch(e.target.value)} />
          </div>
