...
export default function HepsiburadaSettingsPage() {
  const [config, setConfig] = useState<HepsiburadaConfig>({
    merchantId: "",
    username: "",
    password: "",
    testMode: "false",
  ...
  async function fetchConfig() {
    try {
      ...
      const hb = list.find((i: any) => i.service === "hepsiburada");
      if (hb) {
        setIsActive(hb.isActive);
        if (hb.config) {
          setConfig({
            merchantId: hb.config.merchantId || "",
            username: hb.config.username || "",
            password: hb.config.password || "",
            testMode: hb.config.testMode || "false",
          });
        }
      }
    } catch {
  ...
  async function fetchStats() {
    try {
      const [productsRes, ordersRes] = await Promise.all([
        fetch("/api/admin/marketplace/hepsiburada/products?size=1"),
        fetch("/api/admin/marketplace/hepsiburada/orders?size=1"),
      ]);
      const products = await productsRes.json();
      const orders = await ordersRes.json();

      const syncedRes = await fetch("/api/admin/marketplace/hepsiburada/products?size=1&syncStatus=SYNCED");
      const failedRes = await fetch("/api/admin/marketplace/hepsiburada/products?size=1&syncStatus=FAILED");
      const pendingRes = await fetch("/api/admin/marketplace/hepsiburada/products?size=1&syncStatus=PENDING");
  ...
  async function handleTest() {
    ...
    try {
      ...
      await fetch("/api/integrations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ service: "hepsiburada", isActive: true, config }),
      });

      // Kategori çekmeyi test et
      const res = await fetch("/api/admin/marketplace/hepsiburada/categories", {
        method: "POST",
      });
      const data = await res.json();
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Hepsiburada Entegrasyonu</h1>
        <p className="text-muted-foreground mt-1">
          Hepsiburada pazaryeri API bağlantı ayarları ve senkronizasyon durumu
        </p>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Hepsiburada Satıcı Paneli → Hesap Bilgilerim → Entegrasyon Bilgileri&apos;nden alabilirsiniz.
        </p>

        <div className="grid gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Merchant ID (Mağaza ID)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Kullanıcı Adı (Username)</label>
            <input
              type="text"
        ...
        <div className="flex items-center gap-6">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => setConfig((c) => ({ ...c, testMode: e.target.checked ? "true" : "false" }))}
              className="w-4 h-4"
            />
            <span className="text-sm font-medium">Test Modu (SIT)</span>
          </label>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={handleTest}
            disabled={testing || !config.merchantId || !config.username || !config.password}
