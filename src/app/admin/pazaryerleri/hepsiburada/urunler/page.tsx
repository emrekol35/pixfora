...
export default function HepsiburadaProductsPage() {
  ...
  const [hbCategories, setHbCategories] = useState<HBCategory[]>([]);
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/marketplace/hepsiburada/products?${params}`);
  ...
  useEffect(() => {
    ...
    catTimerRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`/api/admin/marketplace/hepsiburada/categories?search=${encodeURIComponent(catSearch)}&all=true&leafOnly=true`);
        const data = await res.json();
        setHbCategories(
          (data.categories || []).map((c: any) => ({ id: c.id, name: c.name, path: c.path, leaf: c.leaf }))
        );
      } catch { setHbCategories([]); }
  ...
  async function handleExport() {
    ...
    setExporting(true);
    try {
      const res = await fetch("/api/admin/marketplace/hepsiburada/products", {
        method: "POST",
  ...
  async function handleBulkConfigure() {
    ...
    setSavingBulk(true);
    try {
      const res = await fetch("/api/admin/marketplace/hepsiburada/products/configure", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          productIds: Array.from(selected),
          hepsiburadaCategoryId: bulkCategoryId,
  ...
  function getStatusBadge(hp: ProductItem["hepsiburadaProduct"]) {
    if (!hp) return <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-xs">Yapılandırılmamış</span>;
    ...
    return (
      <span className={`px-2 py-0.5 rounded text-xs ${colors[hp.syncStatus] || "bg-gray-100 text-gray-500"}`}>
        {labels[hp.syncStatus] || hp.syncStatus}
      </span>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold">Hepsiburada Ürünleri</h1>
        ...
        <div className="flex gap-2">
          {selected.size > 0 && (
            <>
              ...
              >
                {exporting ? "Aktarılıyor..." : `Hepsiburada'ya Aktar (${selected.size})`}
              </button>
            </>
          )}
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50">
            <tr>
              ...
              <th className="text-center p-3 font-medium">HB Durumu</th>
              <th className="text-left p-3 font-medium">HB Kategori/Marka</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              products.map((p) => (
                <tr key={p.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="p-3 text-center">{getStatusBadge(p.hepsiburadaProduct)}</td>
                  <td className="p-3 text-xs text-muted-foreground">
                    {p.hepsiburadaProduct?.hepsiburadaCategoryId
                      ? `Kat: ${p.hepsiburadaProduct.hepsiburadaCategoryId}`
                      : "—"}
                    {p.hepsiburadaProduct?.brandName && (
                      <> / Mrk: {p.hepsiburadaProduct.brandName}</>
                    )}
                    {p.hepsiburadaProduct?.lastError && (
                      <div className="text-red-500 mt-1 truncate max-w-xs" title={p.hepsiburadaProduct.lastError}>
                        ⚠ {p.hepsiburadaProduct.lastError}
                      </div>
                    )}
                  </td>
      ...
      {showBulkModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-card border border-border rounded-lg p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold">Toplu Hepsiburada Yapılandırması</h3>
            <p className="text-sm text-muted-foreground">{selected.size} ürün seçili</p>

            {/* Kategori Arama */}
            <div>
              <label className="block text-sm font-medium mb-1">Hepsiburada Kategorisi</label>
              {selectedCatName && (
              ...
              {hbCategories.length > 0 && (
                <div className="border border-border rounded-lg max-h-40 overflow-y-auto mt-1">
                  {hbCategories.slice(0, 100).map((c) => (
                    <div
                      key={c.id}
                      onClick={() => { setBulkCategoryId(c.id); setSelectedCatName(c.path || c.name); setHbCategories([]); setCatSearch(""); }}
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Hepsiburada&apos;da marka, ürün özelliği olarak gönderilir.
              </p>
            </div>

            <div className="flex gap-3 justify-end">
              <button onClick={() => { setShowBulkModal(false); setCatSearch(""); setHbCategories([]); }}
