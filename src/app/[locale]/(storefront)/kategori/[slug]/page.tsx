...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  let catSeoTitle = category.seoTitle;
  let catSeoDesc = category.seoDescription;
  if (locale !== "tr") {
    const { getEntityTranslations } = await import("@/lib/translations");
    const tr = await getEntityTranslations("category", category.id, locale);
    if (tr.name) catName = tr.name;
    if (tr.seoTitle) catSeoTitle = tr.seoTitle;
    if (tr.seoDescription) catSeoDesc = tr.seoDescription;
  }
  return {
    title: catSeoTitle || catName,
    description: catSeoDesc || `${catName} kategorisindeki urunler`,
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  if (locale !== "tr") {
    const { getEntityTranslations } = await import("@/lib/translations");
    const catTr = await getEntityTranslations("category", category.id, locale);
    if (catTr.name) (category as any).name = catTr.name;
    if (catTr.description) (category as any).description = catTr.description;
    // Alt kategorilerin adlarını çevir
    if (category.children.length > 0) {
      const childTranslations = await import("@/lib/translations").then(m =>
        m.getBulkTranslations("category", category.children.map(c => c.id), locale)
      );
      for (const child of category.children) {
        const tr = childTranslations.get(child.id);
        if (tr?.name) (child as any).name = tr.name;
      }
    }
  }
