...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (locale !== "tr") {
    ...
    const tr = await getEntityTranslations("blogPost", post.id, locale);
    if (tr.title) (post as any).title = tr.title;
    if (tr.excerpt) (post as any).excerpt = tr.excerpt;
  }
...
export default async function BlogPostPage({ params }: Props) {
  ...
  if (locale !== "tr") {
    ...
    const tr = await getEntityTranslations("blogPost", post.id, locale);
    if (tr.title) (post as any).title = tr.title;
    if (tr.content) (post as any).content = tr.content;
    if (tr.excerpt) (post as any).excerpt = tr.excerpt;
  }
